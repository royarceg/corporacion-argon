  ...
  return (
    <>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
        ...
        <button onClick={openCreate} style={{ fontFamily: "Graphik, sans-serif", fontSize: "12px", fontWeight: 500, color: "#fff", backgroundColor: "#000", border: "none", padding: "9px 20px", cursor: "pointer", letterSpacing: "0.04em", textTransform: "uppercase" }}>
          + Nuevo Grupo
        </button>
      </div>
      ...
      {fetching ? (
        ...
        <p style={{ fontFamily: "Graphik, sans-serif", fontSize: "13px", color: "rgba(0,0,0,0.4)", padding: "40px 0" }}>No hay grupos de variantes.</p>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          {groups.map((g) => (
            <div key={g.id} style={{ border: "1px solid rgba(0,0,0,0.1)", padding: "20px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "14px" }}>
                <p style={{ fontFamily: "Graphik, sans-serif", fontSize: "14px", fontWeight: 500, color: "#000", margin: 0 }}>{g.name}</p>
                <div style={{ display: "flex", gap: "8px" }}>
                  <button onClick={() => openEdit(g)} style={{ fontFamily: "Graphik, sans-serif", fontSize: "11px", color: "#000", border: "1px solid rgba(0,0,0,0.2)", background: "none", padding: "5px 12px", cursor: "pointer" }}>Editar</button>
                  <button onClick={() => handleDelete(g.id)} style={{ fontFamily: "Graphik, sans-serif", fontSize: "11px", color: "#9c0f0f", border: "1px solid rgba(0,0,0,0.2)", background: "none", padding: "5px 12px", cursor: "pointer" }}>Eliminar</button>
                </div>
              </div>
              <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                {g.products.map((p) => (
                  <div key={p.product_id} style={{ display: "flex", alignItems: "center", gap: "8px", padding: "6px 12px", backgroundColor: "#f5f4f4", borderRadius: "4px" }}>
                    ...
                    <span style={{ fontFamily: "Graphik, sans-serif", fontSize: "12px", color: "#000" }}>{p.name}</span>
                    {p.color && <span style={{ fontFamily: "Graphik, sans-serif", fontSize: "10px", color: "rgba(0,0,0,0.4)" }}>({p.color})</span>}
                  </div>
                ))}
              </div>
            </div>
        ...
        <>
          ...
          <div style={{ position: "fixed", top: "5%", left: "50%", transform: "translateX(-50%)", backgroundColor: "#fff", width: "560px", maxHeight: "90vh", overflowY: "auto", zIndex: 201, padding: "28px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
              <h3 style={{ fontFamily: "Graphik, sans-serif", fontSize: "16px", fontWeight: 400, margin: 0 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
              <div>
                <p style={{ fontFamily: "Graphik, sans-serif", fontSize: "10px", fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: "rgba(0,0,0,0.5)", margin: "0 0 4px 0" }}>Nombre del grupo</p>
                <input style={inputStyle} value={formName} onChange={(e) => setFormName(e.target.value)} placeholder="ej. Camisa Columbia Hombre" />
              </div>

              <div>
                <p style={{ fontFamily: "Graphik, sans-serif", fontSize: "10px", fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: "rgba(0,0,0,0.5)", margin: "0 0 4px 0" }}>
                ...
                <div style={{ maxHeight: "320px", overflowY: "auto", border: "1px solid rgba(0,0,0,0.1)" }}>
                  {filteredProducts.map((p) => (
                    ...
                    >
                      ...
                      <span style={{ fontFamily: "Graphik, sans-serif", fontSize: "12px", color: "#000", flex: 1 }}>{p.name}</span>
              ...
              {formProductIds.size < 2 && (
                <p style={{ fontFamily: "Graphik, sans-serif", fontSize: "11px", color: "rgba(0,0,0,0.4)", margin: 0 }}>Seleccioná al menos 2 productos para crear un grupo.</p>
              )}
